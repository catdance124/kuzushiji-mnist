from keras.layers import Input, BatchNormalization, Activation, Add, Reshape, Multiply,\
    Conv2D, GlobalAveragePooling2D, Dense, Layer, Lambda, Dropout
...
# -----------------------------------------
# Shake-Shake regularization https://github.com/jonnedtc/Shake-Shake-Keras/blob/master/layers.py
...
def _resblock_shake(n_filters, SE=False, Drop=False):
  """ Residual Block with Shake-Shake regularization and shortcut """
  def f(input):
    x1 = _resblock(n_filters=n_filters, strides=(1,1), SE=SE, Drop=Drop)(input)
    x2 = _resblock(n_filters=n_filters, strides=(1,1), SE=SE, Drop=Drop)(input)
    return _shortcut(input, ShakeShake()([x1, x2]))
  return f